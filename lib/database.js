...
var cloudant_credentials = appEnv.getServiceCreds(/[c|C]loudant/);
log.info('cloudant_credentials', cloudant_credentials)
// Initialize the library with my account.

var dbName = process.env.DB_NAME || 'multimedia-enrichment';

var dbConfig = cloudant_credentials ||  {
...
cdb.db.create(dbName, function (err, res) {
    if (err) { 
    ...
    cdb.use(dbName).insert(views,  function(error, body) {
...
var saveDocument = function saveDocument(id, document, callback) {
  ...
  // First try to insert
  db.insert(document, id, function(error, body) {
    if (error) {
      if (error.statusCode === 409) {
...
var loadDocument = function loadDocument(id, callback) {
  console.log('Trying to load document: ', id);
  db.get(id, callback);
};

// Get all media based on 
var getAllMedia = function getAllMedia(callback) {
  var mediaView = 'new-view-media';
  db.view(viewdoc, 'new-view-media', function(err, result) {
    if (err) {
...
// Get all media based on 
var getAllEpisodes= function getAllEpisodes(callback) {
  var mediaView = 'episodes_short';
  db.view(viewdoc, mediaView, function(err, result) {
    if (err) {
...
var getWholeEpisode = function getWholeEpisode(id, callback) {
  var mediaView = 'episodes2';
  ...
  params.keys.push(id);
  db.view(viewdoc, mediaView, params, function(err, result) {
    if (err) {
      console.log('getEpisodes ERROR', err);
...
var getSegmentMetadata = function getSegmentMetadata(id, callback) {
  var mediaView = 'segments';
  console.log('getSegment Called: '+ id);
  db.view(viewdoc, mediaView, {keys: [id]},  function(err, result) {
    if (err) {
...
var getSegmentMoments = function getSegmentMoments(id, callback) {
  var mediaView = 'moments_by_segment';
...
var getSegmentScenes = function getSegmentScenes(id, callback) {
  var mediaView = 'scenes';
...
var getEpisodeSegments = function getEpisodeSegments(id, callback) {
  var mediaView = 'segments';
...
var query = function query(selector, callback) {
  db.find(selector,function (error, result) {
    callback(error, result);
  })
}

var getCustomEntities = function getCustomEntities(callback) {
  var mediaView = 'custom_entities';
  var params = {
    group: true
  }

  db.view(viewdoc, mediaView, params,  function(error, result) {
    if (error) {
...
var getStats= function getStats(callback) {
  var mediaView = 'summary';
  var params = {
    group: true
  }
  db.view(viewdoc, mediaView, params,  function(error, result) {
...
var getVRClasses = function getVRClasses(callback) {
  var mediaView = 'vr_classes';
  var params = {
    group: true
  }
...
// Get all media based on 
var getSearches= function getSearches(callback) {
  var mediaView = 'searches';
  var params = {
    include_docs: true
  }
  db.view('search', mediaView , params, function(err, result) {
    if (err) {
...
var getSceneMoments = function getSceneMoments(id, callback) {
  var mediaView = 'moments';
