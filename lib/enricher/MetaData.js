...
const isRelevant = (obj, limit) => {
  ...
  let relevance = (limit) ? limit : defaultRelevance
...
const getTags = (alchemyArray, relevance) => {
  ...
  return alchemyArray.filter((obj) => {
     return isRelevant(obj, relevance);
  }).map((obj) => {
    return (obj.text) ? (obj.text): obj.label;
  });
}
...
const generateTags = (data) => {
  ...
  const tags = Object.keys(tagSources).map((key) => {
    ...
    return getTags(data[key], tagSources[key]);
...
//    console.log(data);
    ...
    this.data = Object.assign({},watson_enrichment);
...
//        log.info('STT_DATA? ', data.stt_data);
        ...
        if (data.speakerTimeline && data.speakerTimeline.length === 0 ) {
          this.data.speakerTimeline = sttUtils.generateTranscriptTimeline(this.data.raw_stt_results);
      ...
      this.data.start_time = data.begin ? data.begin : sttUtils.getStartTime(this.data.raw_stt_results);
