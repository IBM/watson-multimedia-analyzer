  ...
  if (sttData) {
    ...
    if (data.hasOwnProperty('timestamps')) {
      ...
      if (data.timestamps[0] && data.timestamps[0].length > 0) {
        start_time = data.timestamps[0][1];
      } else {
        log.debug('getStartTime failed ', data);
      }
    }
  }
  log.debug('getStartTime returning', start_time);
  return start_time;
};
...
const getEndTime = (sttData) => {
  ...
  const end_time_array = sttData.map(stt => stt.results[0].alternatives[0].timestamps).filter((ts) => {
...
//    log.info('a1', a);
 ...
 //    log.info('a2', a);
    // Just reduce down to the biggest timestamp.
     (a[2] > b[2] ? a : b), [0, 0, 0]);
  log.debug('getEndtime: filtered & reduced ', end_time_array);
  return end_time_array[2];
};
...
const findEntityTime = (entity, sttData) => {
  ...
  if (sttData && sttData.length > 0) {
    const data = sttData[0].results[0].alternatives[0];
    // We expect a single word, or a Pair set of entities separated by a space like a name "Hillary Clinton"
