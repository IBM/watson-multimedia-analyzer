...
const getXML = (xmlURI, callback) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (xml.protocol && xml.protocol.search('http') >= 0) {
      ...
      request.get(xmlURI, function(error, response, body) {
        if (error) {
          reject(error);
        } else {
          resolve(body);
...
// Save the data
const saveEpisode = (episode, full_enrichment) => {
  ...
  db.save(episode.guid, {
    ...
    full_enrichment: (full_enrichment) ? full_enrichment : {}
  }, (err, msg) => {
    if (err) {
      log.error(err);
    }
...
const saveMedia = (media) => {
  ...
  return new Promise(function(resolve, reject) {
    ...
    db.save(media.mediaMetadata.guid, media, function(err, resp) {
      if (err) {
...
const processEpisode = (logid, episode, useSTT, useVR, vrRate) => {
  return new Promise(function(resolve, reject) {
    var options = {};
    ...
    if (episode.hasOwnProperty('segments')) {
      ...
      saveEpisode(episode);
      var queue = episode.segments.slice(0);
      ...
      function processQueue() {
        if (queue.length > 0) {
          if (can_process) {
            ...
            processSegment(logid, segment, options)
              .then((s) => {
                ...
                if (s.full_enrichment && s.full_enrichment.transcript) {
                  log.debug('Pushing the enrichment to episode')
                  transcripts.push(s.full_enrichment.transcript);
                }
              })
              .catch((error) => {
          ...
          enrichText({
            ...
            //          log.debug('Episode Enriched', full_enrichment);
            saveEpisode(episode, full_enrichment);
  ...
  generateMediaMetadata(filename, (err, md) => {
    if (err) {
      ...
      if (md.content.filename) {
        log.info(`Copying file... ${md.content.filename} to ${home+media_dir}`);
        fs.copy(md.content.filename, home+media_dir);
        md.content.url = media_dir+path.basename(md.content.filename);
      }
...
/**
 * A segment is essentially one complete 'Media' file.  whatever it is... this will/may be broken up smaller to
 * Scenes and Moments.
 *
 */
...
const processMedia = (logid, media, useSTT, useVR, vrRate) => {
  ...
  var media_type = (media.type) ? media.type : GENERIC_MEDIA;
  var options = {};
  if (typeof useSTT === 'object' && typeof useVR === 'undefined') {
    options = useSTT;
  } else {
  ...
  return new Promise(function(resolve, reject) {
    initMediaProcessor(media, options)
      .on('enricheddata', (data) => {
        //data.type = SCENE;
        // log.debug(segment.title+ ': Finished Enriching Scene: '+ data.guid);
        //  saveScene(data);
      })
      .on('finalenricheddata', (data) => {
        ...
        if (data.by_timeline_enrichment) {
          ...
          for (var i = 0; i < data.by_timeline_enrichment.length; i++) {
            // These are all SCENES.
            var obj = data.by_timeline_enrichment[i];
...
const initMediaProcessor = (mediafile, useSTT, useVR, vrRate) => {
  ...
  let mediaProcessor = new MediaProcessor(null, mediafile);
...
const saveProcessingState = (uuid, message) => {
  if (uuid) {
    const msg = {
      'time': new Date(),
      'uuid': uuid,
...
const processMediaFromGUID = (obj) => {
        ...
        if (err) {
          ...
          if (doc.type === 'episode') {
            ...
            processEpisode(logID, doc.mediaMetadata, obj)
              ...
              .catch((error) => {
                saveProcessingState(logID, 'ERROR: Failed Enriching Episode ' + obj.guid);
              });
            ...
            processSegment(logID, doc.mediaMetadata, obj)
              ...
              .catch((error) => {
                saveProcessingState(logID, 'ERROR: Failed Enriching Segment ' + obj.guid);
              });
          } else {
...
const processMediaFromXML = (uri, useSTT, useVR) => {
  ...
  getXML(uri)
    .then((xml) => {
      ...
      let episodes_to_process = meta.length;
      log.debug('Loaded XML, will process ' + episodes_to_process);
      var episode_queue = meta;
      var can_process = true;
      ...
      function processEpisodeQueue() {
        if (episode_queue.length > 0) {
          if (can_process) {
            // Pull an episode off
            var episode = episode_queue.shift();
            can_process = false;
