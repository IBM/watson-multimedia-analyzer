...
const stt_utils = require('../enricher/STTDataUtils');
...
function fillGaps(timeline) {
  ...
  log.debug('Timeline is: ', timeline.map((t) => {
  ...
  timeline.sort(function(a, b) {
    ...
    // Copy array
  var finalTimeline = timeline.slice(0);
...
class EnrichmentStream extends Writable {
  constructor(options, enrichmentModel, actuallyEnrich) {
     ...
     */
    this.model = (enrichmentModel) ? enrichmentModel : 'TIMED';
  ...
  addSttRawData(data) {
    // Save raw STT Data
    //log.debug(data);
    log.debug(`Adding Raw data for ${stt_utils.getStartTime([data])} -> ${stt_utils.getEndTime([data])}`);
...
//    log.debug('Enriching object: ', objToEnrich);
  ...
  _timelineToSpeakers() {
    ...
    var currentTL = this.textTimeline.slice(0);
    this.textTimeline = [];

    for (var i = 0; i < currentTL.length; i++) {
      var ccObject = currentTL[i];
      //  log.debug('timelineToSpeakers: 2 ', ccObject);
      var newSpeaker = ccObject.text.match(/^>>(.+$)/);
      if (newSpeaker) {
  ...
  _chunkByTimeGap(ccObject) {
    ...
    var elapsedTime = (parseInt(this.textChunk.begin) === 0) ? 0 : parseInt(ccObject.begin) - parseInt(this.textChunk.end);
  ...
  finalEnrichment(callback) {
    ...
    this.timedEnrichment(() => {
      ...
      this.enrichText({
        ...
        this.finalMetadata.full_enrichment = (err) ? err : result;
        ...
        this.visualRecognitionQueue = getTimes(getEnd(this.finalMetadata.by_timeline_enrichment), this.screenshotInterval)
