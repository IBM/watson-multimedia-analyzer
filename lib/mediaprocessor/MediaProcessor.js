...
var sttUsername = process.env.STT_USERNAME;
var sttPassword = process.env.STT_PASSWORD;
var vrKey = process.env.VR_KEY;
//TODO:  Convert to the correct array?
//
var uuid = require('node-uuid');
var EventEmitter = require('events');

var createGuid = function createGuid(filename) {
  return uuid.v1() + '-' + path.basename(filename);
}

var isYoutube = (link) => {
  return (link.search('youtube') >= 0);
}
...
function createMoments(enricheddata, vr) {
  // each Scene...
  //log.debug('createMoments...', enricheddata);
  enricheddata.by_timeline_enrichment.forEach(function(obj) {
    // Each Scene
    ...
    vr.forEach(function(image) {
      log.debug(`Processing Image Time: ${image.images[0].time}`);
      var t = parseInt(ms ? image.images[0].time * 1000 : image.images[0].time);
      //var t = parseInt(image.images[0].time);
      var m = {};
      log.debug(`Converted time to Time: ${t} ${(ms ? 'ms' : 'sec')}`);
      var moment_tone = [];
      if (obj.hasOwnProperty('speaker_tone')) {
        moment_tone = obj.speaker_tone.filter(function(tone) {
          if (tone) {
            return (t > tone.begin && t < tone.end)
          }
        });
      }
      if (t > obj.start_time && t < obj.end_time) {
        ...
        if (moment_tone.length > 0) {
          m.text = moment_tone[0].text;
          m.tone = moment_tone[0];
        }
        log.debug('Returning a Moment: ', m);
        obj.moments.push(m);
      }
    });
    obj.moments.sort(function(a, b) {
      if (a.time < b.time) {
        return -1;
      }
      if (a.time > b.time) {
  ...
  _initEnrichmentStream(model, timegap) {
    ...
    this.enrichmentStream.on('visualrecognition', (data) => {
      ...
      if (this._use_vr && data.length > 0) {
        ...
        vr.visualRecognition(this.mediaMetadata, data, this.vr_api_key)
          .then((visual_recognition) => {
            if (visual_recognition) {
              log.debug('Visual Recognition finished: ' + visual_recognition.length);
              log.debug('creating Moments ... ');
              createMoments(this._finalEnrichedData, visual_recognition);
            } else {
  ...
  _initSTTStream(file) {
    ...
    if (file) {
      try {
        ...
        mediaStreamIn = file;
        if (!this.guid) {
           this.guid = this.mediaMetadata.guid = createGuid(file);
        }
      } catch (e) {
        log.error('failed to open file for reading...', e);
      }
    } else if (typeof this.mediaLink.pipe === 'function') {
      // it is a Readable Stream
      mediaStreamIn = this.mediaLink;
    } else {
        mediaStreamIn = request.get(this.mediaLink)
          .on('error', (err) => {
            log.debug('Error!!!', err);
            this.emit('error', err);
          });
    }
    log.debug('_initSTTStream MediaLink is: ', this.mediaLink);
    log.debug('_initSTTStream SUBTITLES LINK:', this.sub_titles_link);
    log.debug('_initSTTStream DO STT:', this._do_stt);
    */

    var speech_to_text = new SpeechToTextV1();

    this.sttStream = speech_to_text.createRecognizeStream({
