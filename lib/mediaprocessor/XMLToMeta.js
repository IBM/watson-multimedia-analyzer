...
function ItemToMetadata(item) {
  // Some content providers will provide SEGMENTS in the item ( so multiple media files  & info per file)  How do we store it, need to introspect the item.
  //
  log.debug('item: ', item);
  var md = {};
  md.segments = [];
  var media = {};
  var final_metadata = false;
...
module.exports = function XMLtoMeta(xmlFile) {
  var obj = parse(xmlFile);
  var metaDataArray = [];
  var mainObj = obj.root.children[0];
