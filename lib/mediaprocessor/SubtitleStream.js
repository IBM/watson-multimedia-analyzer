...
const stringToMilliSeconds = (time) => {
  var t = time.split(':');
  var inMS = 0;
  inMS = (parseInt(t[0])*3600 + parseInt(t[1])*60 + parseFloat(t[2]))*1000;
  return inMS;
}
  ...
  if (stJSON.hasOwnProperty('root')) {
    ...
    stItems.forEach((p) => {
      ...
      if (subtitles.length > 0) {
        var last_st = subtitles[subtitles.length-1];
        if (st.begin === last_st.begin && st.end === last_st.end) {
          last_st.text = last_st.text + ' ' + st.text;
        } else {
          subtitles.push(st);
        }
      } else {
